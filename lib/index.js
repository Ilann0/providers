...
const baseUrl$a = "https://api.whvx.net";
const headers$4 = {
...
async function comboScraper$a(ctx) {
  ...
  const res = await ctx.fetcher(`${baseUrl$a}/status`, {
    headers: headers$4
  });
...
const novaScraper$1 = makeEmbed({
  ...
  async scrape(ctx) {
    ...
    try {
      const search2 = await ctx.fetcher(`${baseUrl$a}/search?query=${encodeURIComponent(ctx.url)}`);
      if (search2.statusCode === 404) {
        throw new Error("No files found");
      }
      const result = await ctx.fetcher(`${baseUrl$a}/source?resourceId=${encodeURIComponent(search2.url)}`);
...
async function comboScraper$9(ctx) {
  ...
  const res = await ctx.fetcher(`${baseUrl$9}/status`, { headers: headers$3 });
  if (((_a = res.providers) == null ? void 0 : _a.length) === 0) {
    throw new NotFoundError("No providers available");
  }
  const embeds = res.providers.map((provider) => {
...
const getUrl = (videoId) => `https://vidmoly.to/embed-${encodeURIComponent(videoId)}.html`;
const vidMolyScraper = makeEmbed({
  ...
  async scrape(ctx) {
    ...
    const htmlReq = await ctx.proxiedFetcher.full(getUrl(videoID), { headers: headers$1 });
    const html = await htmlReq.body;
    const hlsPlaylistUrl = (_a = html.match(PLAYLIST_URL_REGEX)) == null ? void 0 : _a[1];
    ctx.progress(50);
    if (!hlsPlaylistUrl) {
      throw new NotFoundError("HLS playlist not found");
    }
    const proxiedUrl = `https://doesnmatterwhat.wafflehacker.io/m3u8?url=${encodeURIComponent(hlsPlaylistUrl)}&headers=${encodeURIComponent(JSON.stringify(headers$1))}`;
    ctx.progress(100);
    return {
      stream: [{
        id: "some",
...
const baseUrl$8 = "https://soaper.live";
const universalScraper$8 = async (ctx) => {
  ...
  if (ctx.media.type === "show") {
    ...
    const showPage = await ctx.proxiedFetcher(showLink, { baseUrl: baseUrl$8 });
  ...
  if (!showLink)
    throw new NotFoundError("Content not found");
  const contentPage = await ctx.proxiedFetcher(showLink, { baseUrl: baseUrl$8 });
  ...
  const streamRes = await ctx.proxiedFetcher(infoEndpoint, {
    ...
    body: formData,
    headers: {
      referer: `${baseUrl$8}${showLink}`
    }
  });
  const streamResJson = JSON.parse(streamRes);
  const playlistUrl = `${baseUrl$8}${streamResJson.val}`;
  const backupPlaylistUrl = `${baseUrl$8}${streamResJson.val_bak}`;
  const backupPlayRes = await ctx.proxiedFetcher(backupPlaylistUrl, { headers: { referer: `${baseUrl$8}${showLink}` } });
  const playlistRes = await ctx.proxiedFetcher(playlistUrl, { headers: { referer: `${baseUrl$8}${showLink}` } });
  ...
  if (Array.isArray(streamResJson.subs)) {
    for (const sub of streamResJson.subs) {
      ...
      const fullSubUrl = `${proxyPrefix}${encodeURIComponent(baseUrl$8 + sub.path)}`;
      let language = "";
...
const filelionsScraper = makeEmbed({
  ...
  async scrape(ctx) {
    ...
    if (!proxiedPlaylist || proxiedPlaylist === "")
      throw new Error("filelions file not found");
    return {
      stream: [
        {
          type: "hls",
...
const streamwishScraper = makeEmbed({
  ...
  scrape: async (ctx) => {
    ...
    if (!playlistUrl)
      throw new NotFoundError("Failed to find playlist");
...
const baseUrl$5 = "https://player.autoembed.cc/";
...
const tomAutoEmbedScraper = async (ctx) => {
  ...
  const referer2 = new URL(url);
  referer2.pathname = `${url.searchParams.get("type")}/${url.searchParams.get("id")}`;
  let fileUrl = null;
  try {
    const apiResult = await ctx.proxiedFetcher(url.toString(), { headers: { Referer: referer2.toString() } });
...
async function comboScraper$4(ctx) {
  ...
  const playerPage = await ctx.fetcher(path, { baseUrl: baseUrl$5 });
  const results = [];
  let match;
    ...
    try {
      ...
      [sourcesRegex1, sourcesRegex2].forEach((regex) => {
        ...
        try {
          ...
          sourcesArr.filter((s) => {
            var _a, _b;
            return !s.label || !((_b = (_a = s.label).toLowerCase) == null ? void 0 : _b.call(_a)) || s.label.toLowerCase().startsWith("eng");
...
const baseUrl$4 = "https://catflix.su";
...
const baseUrl$1 = "https://arbplus.net";
async function comboScraper$1(ctx) {
  ...
  iframePage$(".dropdown-item").each((_, element) => {
    ...
    if (link == null ? void 0 : link.startsWith("https:"))
      links.push(link);
  });
  const embeds = [];
